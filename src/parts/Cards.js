...
class Cards extends Component {
    constructor(props) {
        ...
        this.Gallery = null
        this.Team = props.refs
        this.Cards = this.Team.cards
    ...
    toLeft(){
        if(this.state.scrollActive){
            let index = this.state.currentCardBlock - 1

            if(index >= 0){
                this.setState({
                    scrollActive: false,
                    currentCardBlock: index
                });

                gsap.to(this.Gallery,{scrollTo:{x: this.Cards[index], offsetX: 0},
                    onComplete:()=>{
                        this.setState({
                            scrollActive:true,
                        })
    ...
    toRight(){
        if(this.state.scrollActive){
            ...
            if(this.Cards.length > index) {
                ...
                gsap.to(this.Gallery,{scrollTo:{x: this.Cards[index]},
                    onComplete:()=>{
                        this.setState({
                            scrollActive:true,
                        })
    ...
    render() {
        return (
            <div className="gallery" ref={el => this.props.refs.gallery = el}>
